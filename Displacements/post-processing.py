...
def ads_and_surf_plot():
    ...
    Eads1 = (EOsurf1 - Esurf1 - count*(0.5)*(EO2+correction)) # units are eV/ O
    Esucc = (EOsurf1 - EsurfO - (0.5)*(EO2+correction))
    ...
    fig,ax = plt.subplots()
    plt.plot(num, Eads1,color="k")
    ax.set_xticks(xticks)
    ax.set_xticklabels(xlabels)
    ax.set_xticks(xticks+(step/Nsurf), minor=True)
    plt.xlim(1/Nsurf,2)
    plt.ylabel("Adsorbtion Energy (eV)")
    plt.xlabel("Coverage (ML)")

    plt.savefig('ads-energy.png',dpi=300,bbox_inches="tight")

    fig,ax = plt.subplots()
    plt.plot(num,EOsurf1/(Natoms+count),color="k")
    ax.set_xticks(xticks)
    ax.set_xticklabels(xlabels)
    ax.set_xticks(xticks+(step/Nsurf), minor=True)
    plt.xlim(1/Nsurf,2)
    plt.ylabel("Electronic Energy (eV/atom)")
    plt.xlabel("Coverage (ML)")
    plt.savefig("electronic-energy.png",dpi=300,bbox_inches="tight")
